...
class Node(QtWidgets.QGraphicsItem):
    def __init__(self, x, y, val):
        ...
        self.val = val[0]  # set node value
        self.highlighted = False
        self.selected = False
        self.heuristic = val[1]  # heuristic value of node

    def paint(self, painter, option, widget):

        if self.selected:
            # if the node is selected paint it red
            painter.setPen(QtCore.Qt.green)
            painter.setBrush(QtGui.QColor(255, 50, 0, 255))
        elif self.highlighted:
            # if the node is highlighted paint it green
            painter.setPen(QtCore.Qt.green)
            painter.setBrush(QtGui.QColor(165, 155, 0, 255))
        else:
            # otherwise paint it orange
        ...
        painter.setFont(QtGui.QFont('Decorative', (4 / len(str(self.val)) + 5)))
        painter.drawText(QtCore.QRect(self.x, self.y, 40, 40), QtCore.Qt.AlignCenter, self.val + self.heuristic)
...
class Edge(QtWidgets.QGraphicsItem):
    def __init__(self, node1, node2, weight, directed):
        ...
        self.midx = (self.x1 + self.x2) / 2  # find midpoint x coordinate of edge
        self.midy = (self.y1 + self.y2) / 2  # find midpoint y coordinate of edge
    ...
    def get_directed_arrow_points(self, x1, y1, x2, y2, d):
        ...
        v1 = x1 - x2  # x coordinate for vector between points
        v2 = y1 - y2  # y coordinate for vector between points

        # to get unit vector requires: u = v/|v|
        dom = math.sqrt(math.pow(v1, 2) + math.pow(v2, 2))  # = |v|

        new_x = v1 / dom  # unit vector x component
        new_y = v2 / dom  # unit vector y component
    ...
    def paint(self, painter, option, widget):
        ...
        if self.directed:  # if edge is part of a digraph
            ...
            painter.drawText(self.midx - 5 * (len(self.strWeight)), self.midy,
                             self.strWeight)  # draw weight near mipoint of edge
...
class GraphScene(QtWidgets.QGraphicsScene):
    def __init__(self, digraph):
        ...
        self.current_path_algo = 'UCS'  # set current path algorithm being used to DIJKSTRA
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() != QtCore.Qt.LeftButton:  # if right button pressed
            self.select_node(event)  # call selected node function
            return
    ...
    def display_path(self):
        if self.current_path_algo == 'PRIMS':  # if current path algorithm is prims
            ...
            if self.check_selected(2):  # if nodes are selected run Dijkstra between them
            ...
            if self.check_selected(2):  # if nodes are selected run dfs between them
                ...
                self.InvalidInMsg.setText('Must select 2 nodes to find shortest path')
                self.InvalidInMsg.exec_()
        elif self.current_path_algo == 'Greedy':  # if current algorithm is greedy
            if self.check_selected(2):  # if nodes are selected run greedy between them
                self.show_Greedy(self.selected[0].val, self.selected[1].val)
                self.deselect_nodes()  # deselect nodes
            else:  # else print error
                self.InvalidInMsg.setText('Must select 2 nodes to find shortest path')
                self.InvalidInMsg.exec_()
        elif self.current_path_algo == 'A*':  # if current algorithm is a*
            if self.check_selected(2):  # if nodes are selected run a* between them
                self.show_AStar(self.selected[0].val, self.selected[1].val)
                self.deselect_nodes()  # deselect nodes
            else:  # else print error
                self.InvalidInMsg.setText('Must select 2 nodes to find shortest path')
                self.InvalidInMsg.exec_()
    ...
    def add_node(self, event):
        ...
        node_val, sure = QtWidgets.QInputDialog.getText(QtWidgets.QWidget(), 'Node Name',
                                                        'Enter node name:')  # use dialog to get node value to be added

        if sure:  # dialog value was input
            if len(str(node_val)) < 5 and len(str(node_val)) > 0:  # if input was between 1 and 4 characters
                ...
                if ok:
                    h = self.check_Heuristic(heuristic_val)

                    if h != -1:
                        text = (str(node_val), "\nh="+str(h))
                        node = Node(x - 20, y - 20, text)  # create a new node at the given x and y coordinates
                        ...
                        if ok:
                            self.graph.add_heuristic(node.val, h)

                    else:
                        self.InvalidInMsg.setText(
                            'Re-enter Node Info')  # print message if invalid dialog input
                        self.InvalidInMsg.exec_()
            else:
                self.InvalidInMsg.setText(
    ...
    def add_edge(self, node1_val, node2_val, weight):
        ...
        try:
            numWeight = float(weight)  # try to convert weight to a float
    ...
    def show_shortest_path_bellman_ford(self, from_node_val, to_node_val):
        ...
        if path == None:  # if path is not possible
            if short_path_info[1] > 0:  # check if due to there being no path to end node
                self.InvalidInMsg.setText(
                    'No path exists between nodes "' + str(from_node_val) + '" and "' + str(to_node_val) + '"')
                self.InvalidInMsg.exec_()  # show relevant message
    ...
    def show_mst_prims(self):
        ...
        mst_edges = path_alg.prims(self.graph)  # call prims algorithm on underlaying graph
        ...
        for edge in mst_edges:
            if (edge[1], edge[2]) in self.edges:  # and edge exists between current node value and next in path
                ...
                self.edges[(
                    edge[2],
                    edge[1])].highlighted = True  # else the edge exists as being from next in path to current node
            dist = dist + edge[0]
    ...
    def show_DFS(self, from_node_val, to_node_val):
        ...
        if from_node_val not in self.nodes :  # nodes for path not in nodes dictionary
            self.InvalidInMsg.setText('Invalid node value input')
            self.InvalidInMsg.exec_()  # show message and exit
            return
        to_node_val = to_node_val.split()
        for n in to_node_val:
            if n not in self.nodes:  # nodes for path not in nodes dictionary
                self.InvalidInMsg.setText('Invalid node value input')
                self.InvalidInMsg.exec_()  # show message and exit
                return

        short_path_info = path_alg.DepthFirst(self.graph, from_node_val, to_node_val)[
            0]  # call dfs algorithm on underlaying graph
    ...
    def show_BFS(self, from_node_val, to_node_val):
        ...
        short_path_info = path_alg.BreadthFirst(self.graph, from_node_val, to_node_val)[
            0]  # call bfs algorithm on underlaying graph
    ...
    def show_UCS(self, from_node_val, to_node_val):
        ...
        path = None
        to_node_val = to_node_val.split()
        if from_node_val not in self.nodes:
            self.InvalidInMsg.setText('Invalid node value input')
            self.InvalidInMsg.exec_()  # show message and exit
            return
        for n in to_node_val:
        ...
        short_path_info = path_alg.UCS(self.graph, from_node_val, to_node_val)[
            0]  # call ucs algorithm on underlying graph

        visited = path_alg.UCS(self.graph, from_node_val, to_node_val)[
            0][2]
    ...
    def show_AStar(self, from_node_val, to_node_val):
        self.delete_shortest_path()  # delete shortest path of currently displayed
        path = None
        ...
        short_path_info = path_alg.Astar(self.graph, from_node_val, to_node_val)[
            0]  # call a* algorithm on underlying graph

        if short_path_info[1] < 0:
            self.InvalidInMsg.setText('Greedy Search requires heuristic to be positive')
            self.InvalidInMsg.exec_()  # show message and exit
            return
        path = short_path_info[2]  # get the path list from running that path search
        ...
        if self.digraph: self.overlay_highlighted()
        # elif self.graph: self.overlay_highlighted()
        self.update()
        self.path_displayed = (
            True, from_node_val, to_node_val, str(short_path_info[1]))  # reset path displayed information
    ...
    def show_Greedy(self, from_node_val, to_node_val):
        self.delete_shortest_path()  # delete shortest path of currently displayed
        path = None
        ...
        short_path_info = path_alg.Greedy(self.graph, from_node_val, to_node_val)[
            0]  # call greedy algorithm on underlying graph
    ...
    def reset_path(self):
        ...
        if self.current_path_algo == 'PRIMS':  # if current algorithm is prims
            ...
            self.show_UCS(self.path_displayed[1], self.path_displayed[2])  # run ucs
        elif self.current_path_algo == 'BFS':  # if current algorithm is bfs
            self.show_BFS(self.path_displayed[1], self.path_displayed[2])  # run bfs
        elif self.current_path_algo == 'DFS':  # if current algorithm is dfs
            self.show_DFS(self.path_displayed[1], self.path_displayed[2])  # run dfs
        elif self.current_path_algo == 'Greedy':  # if current algorithm is greedy
            self.show_Greedy(self.path_displayed[1], self.path_displayed[2])  # run greedy
        elif self.current_path_algo == 'A*':  # if current algorithm is a*
            self.show_AStar(self.path_displayed[1], self.path_displayed[2])  # run a*
    ...
    def check_Heuristic(self, heur_val):
        if len(str(heur_val)) > 0:
            try:
                h = int(heur_val)  # try to convert heuristic to an int
            except ValueError:  # value error exception if unable to cast weight as a float
                self.InvalidInMsg.setText('heuristic must be a number')
                self.InvalidInMsg.exec_()
                return -1
            if h < 0:
                self.InvalidInMsg.setText("Heuristic must be positive or equal to 0")
                self.InvalidInMsg.exec_()
                return -1
            return h
        else:
            self.InvalidInMsg.setText("You must write a heuristic value for the node")
            self.InvalidInMsg.exec_()
            return -1
